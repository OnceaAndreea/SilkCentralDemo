...
const getExecutableFile = async (
    ...
    for (const testName of testNames) {
        ...
        let iterations: { [key: string]: string }[] = await getParameters(test, testContainerAppModule, testSuiteId, runSuiteId);
...
const testSuiteId = process.argv[4];
const runSuiteId = process.argv[5];
...
getExecutableFile(testsToRun, jarPath, nunitDirectories, testSuiteId, runSuiteId, credentials)
