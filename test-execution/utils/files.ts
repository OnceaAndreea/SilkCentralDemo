...
const getPredefinedParameters = async (
    ...
    predefinedParameters["#sctm_regular_execdef_run_id"] = runSuiteId;
    if (testContainerAppModule.sc_product_name_udf) {
        predefinedParameters['#sctm_produt'] = testContainerAppModule.sc_product_name_udf;
    }
...
const getTestSuiteRelatedParameters = async (testSuite: OctaneTestSuite): Promise<{ [key: string]: string }> => {
    ...
    if (testSuite.silk_release_build_udf !== undefined) {
        testSuiteParameters["#sctm_build"] = extractBuildVersion(testSuite.silk_release_build_udf.name);
    }
    if (testSuite.silk_release_version_udf !== undefined) {
...
const getParameters = async (test: OctaneTest,
    ...
    let predefinedParams: { [key: string]: string } = await getPredefinedParameters(test, testContainerAppModule, testSuiteId, runSuiteId);
