...
const getTestSuiteById = async (testSuiteId: string) : Promise<OctaneTestSuite> => {
    let query;
    query = Query.field("id").equal(testSuiteId);
    const octaneResponse = await octane
    ...
    if (octaneResponse.data[0] === undefined) {
        throw new Error(
            `Not found! Test Suite with id ${testSuiteId} does not exist in Octane.`
        );
    }
    return <OctaneTestSuite>{
        ...octaneResponse.data[0],
        silk_release_build_udf: await extractOctaneListNode(<OctaneListNode>(
            octaneResponse.data[0].silk_release_build_udf
        ), testSuiteId),
        silk_release_version_udf: await extractOctaneListNode(<OctaneListNode>(
            octaneResponse.data[0].silk_release_version_udf
        ), testSuiteId),
        sc_exec_keywords_udf: await extractOctaneListNodes(<OctaneListNode[]>(
...
const getOctaneListNodesFromIds = async (
    ...
    if (octaneResponse.data === undefined) {
        throw new Error(
            `Not found! Could not get Execution Keywords of Test Suite with id ${testSuiteId} .`
        );
    }
...
const extractOctaneListNodes = async (
    ...
    return await getOctaneListNodesFromIds(getOctaneListNodeIds(octaneListNodes), testSuiteId);
}
...
const extractOctaneListNode = async (
    ...
    let extractedListNode: OctaneListNode[] = await extractOctaneListNodes(listNode, testSuiteId);
